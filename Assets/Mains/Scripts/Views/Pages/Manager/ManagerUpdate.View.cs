...
namespace YNL.JAMOS
{
    public partial class ManagerUpdate
    {
        private class View : PageView
        {
            ...
            {
                _b.OnPropertyValueChanged?.Invoke("PublicationDate", evt.newValue.RemoveWord("(dd/mm/yyyy)").Trim());
            }
            ...
            {
                _b.OnProductUpdateApplied?.Invoke();
            }

            private void OnDataFieldReseted(ProductWrapper wraper)
            {
                _genreTypeField.ResetValue();
                _genreTypeField.RecreateGenreItems(wraper.ProductType);
                _productPropertyField.RecreatePropertyItems(wraper.ProductType);
